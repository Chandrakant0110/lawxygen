        ...
        <div className="bg-white border rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300">
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {servicesList.map((service, index) => (
                <div key={index} className="flex items-start group">
                  <CheckCircle className={`w-5 h-5 ${colorClass} mr-2 flex-shrink-0 mt-0.5 group-hover:scale-110 transition-transform`} />
                  <div>
            ...
            <div className="mt-8 flex justify-center">
              <Button asChild className={`${colorClass.replace('text-', 'bg-').replace('100', '600')} hover:${colorClass.replace('text-', 'bg-').replace('100', '700')} text-white transition-colors`}>
